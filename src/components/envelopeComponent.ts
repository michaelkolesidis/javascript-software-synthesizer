...
type CreateEnvelopeSliderOptions = Partial<MultiSliderOptions & BaseInterfaceOptions>;
type CreateEnvelopeSelectOptions = Partial<SelectOptions & BaseInterfaceOptions>;
...
const createSlider = (component: HTMLElement, options: CreateEnvelopeSliderOptions) => {
...
// @todo refactor
const createAttackSelect = (component: HTMLElement, options: CreateEnvelopeSelectOptions) => {
	const element = component.children[1].children[1];
	assertInstanceOf(element, HTMLElement);
	return new Nexus.Select(element, options);
};

const createDecaySelect = (component: HTMLElement, options: CreateEnvelopeSelectOptions) => {
	const element = component.children[1].children[3];
	assertInstanceOf(element, HTMLElement);
	return new Nexus.Select(element, options);
};

const createReleaseSelect = (component: HTMLElement, options: CreateEnvelopeSelectOptions) => {
...
export default function EnvelopeComponent(
	...
	options.selectAttack = Object.assign(defaultOptions?.selectAttack, options.selectAttack);
	options.selectDecay = Object.assign(defaultOptions.selectDecay, options.selectDecay);
	options.selectRelease = Object.assign(defaultOptions.selectRelease, options.selectRelease);
