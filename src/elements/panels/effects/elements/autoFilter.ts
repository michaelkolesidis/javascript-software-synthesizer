...
type AutoFilterUIOptions = {
	[K in keyof Pick<AutoFilterOptions, 'depth' | 'frequency' | 'octaves'>]: CreateDialOptions;
};
...
function render() {
	const [wrapper, toggleWrapper, contentWrapper] = createEffectElements(id, 'Auto Filter');
	...
	interfaces.octaves = NumberDialComponent(contentWrapper, ids.octaves, labels.octaves, options.octaves);
