...
function render() {
	const [wrapper, toggleWrapper, contentWrapper] = createEffectElements(id, 'Bit Crusher');

	interfaces.toggle = new Nexus.Toggle(toggleWrapper, defaultToggleOptions);

	interfaces.bits = NumberDialComponent(contentWrapper, ids.bits, labels.bits, options.bits);
